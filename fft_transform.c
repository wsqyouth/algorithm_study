...
int NFFT=(int)pow(2.0,ceil(log((double)Nx)/log(2.0)));      /*输入序列的大小，在本程序中仅限2的次幂*/
...

{
	...
	double sumCos,averageCos; //去除cos直流分量 

    double sumSin,averageSin; //去除Sin直流分量 
...

{
	...
	for(i=0;i< log(NFFT)/log(2) ;i++)        /*一级蝶形运算 stage */

	{   

		l=1<<i;

		for(j=0;j<NFFT;j+= 2*l )     /*一组蝶形运算 group,每个group的蝶形因子乘数不同*/
...

{
	...
	W=(complex *)malloc(sizeof(complex) * NFFT);  //生成变换核

	for(i=0;i<NFFT;i++)

	{

		W[i].real=cos(2*PI/NFFT*i);   //用欧拉公式计算旋转因子
